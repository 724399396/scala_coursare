...
object FunSets {
  ...
  def union(s: Set, t: Set): Set = new Set {
    override def apply(v1: Int): Boolean = s(v1) || t(v1)
  }
  ...
  def filter(s: Set, p: Int => Boolean): Set = new Set {
    override def apply(v1: Int): Boolean = s(v1) && p(v1)
  }
  ...
  def map(s: Set, f: Int => Int): Set = {
    def help(newSet: Set, ele: Int): Set = {
      if (ele > bound) newSet
      else if (s(ele)) help(union(newSet, singletonSet(f(ele))), ele + 1)
      else help(newSet, ele+1)
    }
    help(x => false, -bound)
  }
